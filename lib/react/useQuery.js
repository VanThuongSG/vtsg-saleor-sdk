...
const useQuery = (query, variables = {}, options = {}) => {
    ...
    const setData = react_1.default.useCallback((data) => {
        if (!apollo_utilities_1.isEqual(data, prevDataRef.current)) {
            prevDataRef.current = data;
            setResult({ data, error: null, loading: false });
        }
        else {
            setResult(previousResult => (Object.assign(Object.assign({}, previousResult), { loading: false })));
        }
    }, []);
    const { unsubscribe, setOptions, refetch: _refetch, loadMore: _loadMore, } = react_1.default.useMemo(() => saleor.legacyAPIProxy[query](variables, Object.assign(Object.assign({}, options), { onError: (error) => setResult(previousResult => (Object.assign(Object.assign({}, previousResult), { error, loading: false }))), onUpdate: (data) => {
            setData(data);
